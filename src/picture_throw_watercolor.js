...
const PictureThrowWaterColor = () => {
  ...
  const createExplosion = (img, imgCanvas, x, y, imgSize) => {
    ...
    //   particles.push({
    //     x: x + (randomX - img.width / 2) / img.width * 1000,
    //     y: y + (randomY - img.height / 2) / img.height * 800,
    //     dx: 0.7,
    //     dy: 0.7, //img.height * (Math.random()-0.5) * 0.005,
    //     size: Math.random() * 5 + 10, //(imgSize / 30),
    ...
    for (let i = 0; i < 53; i++) {
        for (let j = 0; j < 53; j++){
            const randomX = Math.floor(i * img.width / 53 + Math.random() * 53);
            const randomY = Math.floor(j * img.height / 53 + Math.random() * 53);
      
            particles.push({
              x: x + (randomX - img.width / 2) / img.width * 800,
              y: y + (randomY - img.height / 2) / img.height * 600,
              dx: 0.32,
              dy: 0.32, //img.height * (Math.random()-0.5) * 0.005,
              size: Math.random() * 5 + 10, //(imgSize / 30),
              life: Math.random() * 30 + 30,
              img: img,
              imgX: randomX,
